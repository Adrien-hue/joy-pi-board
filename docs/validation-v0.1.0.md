...
| B-04 | Unknown schema string; missing/non-string version; invalid/trailing JSON; wrong types/ranges; duplicate/unknown members; malformed UTF-8; every required key omitted in turn versus permitted explicit null; forbidden object nulls; `issues: null`; internal_error issue code; missing/duplicate/reordered issues; unsorted/duplicate/oversized interfaces and mismatched firmware issues; all rejected with exact classification. | S01 |
