...
- The current assignment authorizes [Sprint 00](docs/sprint-00.md) only: the build shell, locked tools, useful tests and CI. Do not start S01–S04: no Health client/validator/parser, overview API, cache, dashboard, packaging or deployment. The earlier documentation-only restriction was superseded by the user's explicit S00 instruction.
